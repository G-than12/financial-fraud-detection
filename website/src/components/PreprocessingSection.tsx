...
export const PreprocessingSection: React.FC = () => {
  ...
  return (
    <section id="preprocessing" className="py-20 border-t border-zinc-200/80 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {PREPROCESSING_STEPS.map((step, idx) => (
            <button
              ...
              className={`p-3.5 rounded-xl border text-left transition-all relative ${
                activeStep === idx
                  ? 'bg-zinc-900 border-zinc-900 text-white shadow-elevated'
                  : 'bg-[#fafaf9] border-zinc-200 text-zinc-700 hover:bg-zinc-100/80'
              }`}
            >
              <div className="flex items-center justify-between mb-1.5">
                <span
                  className={`text-[11px] font-mono font-bold ${
                    activeStep === idx ? 'text-emerald-400' : 'text-zinc-400'
                  }`}
                >
                  STEP {step.step}
                </span>
              ...
              <div
                className={`text-xs font-semibold line-clamp-1 ${
                  activeStep === idx ? 'text-white' : 'text-zinc-900'
                }`}
              >
                {step.title.split('&')[0]}
              </div>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-[#fafaf9] p-6 sm:p-8 shadow-subtle">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            
            <div className="lg:col-span-7 space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100/70 border border-emerald-200 text-xs font-mono font-semibold text-emerald-800">
                <span>STAGE {PREPROCESSING_STEPS[activeStep].step} OF 06</span>
              </div>
              
              <h3 className="text-2xl font-bold text-zinc-950 font-sans">
              ...
              <div className="flex items-center gap-2 text-xs font-mono text-emerald-700 bg-emerald-50 px-3 py-2 rounded-lg border border-emerald-100">
