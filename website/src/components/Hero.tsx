        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-4">
              <a
                href="#overview"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-zinc-900 dark:bg-emerald-600 text-white text-sm font-semibold hover:bg-zinc-800 dark:hover:bg-emerald-500 transition-all shadow-subtle hover:translate-y-[-1px]"
              >
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200 text-sm font-semibold hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600 transition-all shadow-subtle hover:translate-y-[-1px]"
              >
          ...
          <div className="lg:col-span-5 relative">
            <div className="relative rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-6 shadow-elevated overflow-hidden">
              ...
              <div className="flex items-center justify-between pb-4 mb-4 border-b border-zinc-100 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 text-xs font-mono text-zinc-400 dark:text-zinc-500 font-medium">PCA_2D_Projection.svg</span>
                </div>
                <span className="text-[11px] font-mono text-emerald-600 dark:text-emerald-400 font-medium px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800/80">
                  k = 3 Clusters
                </span>
              </div>

              {/* Coordinate Plot SVG */}
              <div className="relative w-full aspect-square max-h-[340px] bg-zinc-50/50 dark:bg-zinc-950/80 rounded-xl border border-zinc-100 dark:border-zinc-800/80 p-4 flex items-center justify-center">
                
                {/* SVG Coordinate Grid */}
                <svg viewBox="0 0 300 300" className="w-full h-full overflow-visible">
                  {/* Axis lines */}
                  <line x1="20" y1="150" x2="280" y2="150" stroke="currentColor" className="text-zinc-200 dark:text-zinc-800" strokeWidth="1.5" strokeDasharray="3 3" />
                  <line x1="150" y1="20" x2="150" y2="280" stroke="currentColor" className="text-zinc-200 dark:text-zinc-800" strokeWidth="1.5" strokeDasharray="3 3" />

                  {/* Cluster 0 Points (Sky) */}
                  <g className="opacity-70 dark:opacity-80">
                  ...
                  <g className="opacity-70 dark:opacity-80">
                    ...
                    <circle cx="158" cy="110" r="3.5" fill="#6366f1" />
                    {/* Centroid 2 */}
                    <polygon points="150,85 156,97 144,97" fill="#6366f1" stroke="#ffffff" strokeWidth="1.5" />
                  </g>

                  {/* Axis labels */}
                  <text x="270" y="165" fill="currentColor" className="text-zinc-400 dark:text-zinc-500" fontSize="10" fontFamily="monospace">PC1</text>
                  <text x="156" y="32" fill="currentColor" className="text-zinc-400 dark:text-zinc-500" fontSize="10" fontFamily="monospace">PC2</text>
                </svg>

                {/* Floating Metric Pill 1 */}
                <div className="absolute top-3 left-3 bg-white/95 dark:bg-zinc-900/90 border border-zinc-200/80 dark:border-zinc-800 rounded-lg px-3 py-1.5 shadow-subtle flex items-center gap-2">
                  <span className="text-[11px] font-mono text-zinc-500 dark:text-zinc-400">Silhouette Score:</span>
                  <span className="text-xs font-mono font-bold text-zinc-900 dark:text-zinc-100">{PROJECT_METADATA.summary.silhouetteScore}</span>
                </div>

                {/* Floating Metric Pill 2 */}
                <div className="absolute bottom-3 right-3 bg-white/95 dark:bg-zinc-900/90 border border-zinc-200/80 dark:border-zinc-800 rounded-lg px-3 py-1.5 shadow-subtle flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                  <span className="text-[11px] font-mono text-zinc-500 dark:text-zinc-400">Champion Acc:</span>
                  <span className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400">{(PROJECT_METADATA.summary.bestAccuracy * 100).toFixed(2)}%</span>
                </div>
              </div>

              {/* Legend row */}
              <div className="grid grid-cols-3 gap-2 pt-4 mt-2 text-center text-xs font-mono">
                <div className="p-2 rounded-lg bg-sky-50/50 dark:bg-sky-950/30 border border-sky-100 dark:border-sky-900/50">
                  <div className="w-2 h-2 rounded-full bg-sky-600 mx-auto mb-1" />
                  <span className="text-zinc-600 dark:text-zinc-300">Cluster 0 (555)</span>
                </div>
                <div className="p-2 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50">
                  <div className="w-2 h-2 rounded-full bg-emerald-600 mx-auto mb-1" />
                  <span className="text-zinc-600 dark:text-zinc-300">Cluster 1 (690)</span>
                </div>
                <div className="p-2 rounded-lg bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50">
                  ...
                  <span className="text-zinc-600 dark:text-zinc-300">Cluster 2 (700)</span>
                </div>
              </div>
